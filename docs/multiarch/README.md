...
- AArch64 `mmap` 当前支持匿名私有映射与严格的 `MAP_SHARED | MAP_ANONYMOUS`；共享后备在 fork 后双向可见，而权限、解除映射和固定替换仍归各地址空间独立管理。ELF 装载器虽已按整页文件内容、重叠 `PT_LOAD` 顺序、页权限、文件来源、BSS 补零和私有文件页首次写 COW 建立后续文件映射所需的页级基础，但这些 ELF 页仍在 exec 时装入，不等同于按 fault 延迟读取的通用文件映射。普通 AArch64 文件 `mmap`、按 inode/偏移共享的页缓存、`MAP_SHARED_VALIDATE`、脏页回写、截断和 EOF `SIGBUS` 尚未实现。i386 匿名共享映射的原位 `mremap` 扩展尚无可供多个既存 `mm` 共同引用的单一 shmem 后备，因此当前明确拒绝该操作，避免把彼此独立的 host 内存伪装成同一 futex 后备。`MADV_DONTNEED` 会清零匿名私有映射或已分配 brk 页；匿名共享页保留共同后备内容，但当前模型尚不表达可单独驱逐的 PTE/RSS 驻留状态。
