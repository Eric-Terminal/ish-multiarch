...
static dword_t encode(bool upper, byte_t element_size, byte_t shift,
        byte_t rn, byte_t rd) {
...
static void assert_decode(dword_t word, bool upper,
    ...
    struct aarch64_decoded instruction = decode(word);
    assert(instruction.opcode == (upper ?
            AARCH64_OP_ADVSIMD_SSHLL2 : AARCH64_OP_ADVSIMD_SSHLL));
...
static void test_decode_and_neighbors(void) {
    assert_decode(UINT32_C(0x0f08a420), false, 1, 0, 1, 0);
    assert_decode(UINT32_C(0x4f0fa4e6), true, 1, 7, 7, 6);
    assert_decode(UINT32_C(0x0f10a528), false, 2, 0, 9, 8);
    assert_decode(UINT32_C(0x4f1fa5ee), true, 2, 15, 15, 14);
    assert_decode(UINT32_C(0x0f20a630), false, 4, 0, 17, 16);
    assert_decode(UINT32_C(0x4f3fa6f6), true, 4, 31, 23, 22);
    assert_decode(UINT32_C(0x0f20a5ef), false, 4, 0, 15, 15);

    static const byte_t element_sizes[] = {1, 2, 4};
    unsigned legal = 0;
    for (unsigned upper = 0; upper < 2; upper++) {
        for (unsigned size_index = 0;
                size_index < sizeof(element_sizes) / sizeof(element_sizes[0]);
                size_index++) {
            ...
            for (unsigned shift = 0; shift < element_size * 8; shift++) {
                for (unsigned rn = 0; rn < 32; rn++) {
                    for (unsigned rd = 0; rd < 32; rd++) {
                        assert_decode(encode(upper != 0, element_size,
                                (byte_t) shift, (byte_t) rn, (byte_t) rd),
                                upper != 0, element_size, (byte_t) shift,
                                (byte_t) rn, (byte_t) rd);
                        legal++;
                    }
                }
...
static qword_t reference_lane(
        qword_t value, byte_t source_size, byte_t shift) {
...
static union aarch64_vector_reg expected_result(
        const union aarch64_vector_reg *source, bool upper,
    ...
    for (byte_t lane = 0; lane < lanes; lane++) {
        ...
        write_lane(&result, source_size * 2, lane,
                reference_lane(value, source_size, shift));
    }
    return result;
}
        ...
        for (unsigned size_index = 0;
            ...
            for (unsigned shift = 0; shift < element_size * 8; shift++) {
                ...
                union aarch64_vector_reg expected = expected_result(
                        &source, upper != 0, element_size, (byte_t) shift);
                ...
                execute_instruction(&cpu, encode(upper != 0, element_size,
                        (byte_t) shift, 1, 2));
