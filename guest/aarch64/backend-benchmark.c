...
static void write_subs_extended_program(
        byte_t code[GUEST_MEMORY_PAGE_SIZE]) {
    put_instruction(code, INSTRUCTION_AND_W2_W1_FF);
    put_instruction(code + 4, INSTRUCTION_SUBS_WZR_W2_W1_UXTB);
...
static void write_msub_program(
        byte_t code[GUEST_MEMORY_PAGE_SIZE]) {
    put_instruction(code, INSTRUCTION_MOV_W6_W2);
    put_instruction(code + 4, INSTRUCTION_MSUB_W6_W6_W5_W10);
    // MSUB 保留 Z=0 时，CSINC 才会在每轮增加 X1。
    put_instruction(code + 8, INSTRUCTION_CSINC_X1_X1_X1_EQ);
    put_instruction(code + 12, INSTRUCTION_SUBS_X4_X4_1);
    put_instruction(code + 16, encode_conditional_branch(-16, 1));
    put_instruction(code + 20, INSTRUCTION_SVC);
}

static void write_load_simd_pair_program(
        byte_t code[GUEST_MEMORY_PAGE_SIZE]) {
    put_instruction(code, INSTRUCTION_ADD_SP_X3_300);
    put_instruction(code + 4, INSTRUCTION_LDP_Q30_Q29_SP_80);
    // 将加载结果落回独立区域，避免基准只依赖后端间状态互比。
    put_instruction(code + 8, INSTRUCTION_STP_Q30_Q29_X3_200);
    put_instruction(code + 12, INSTRUCTION_CSINC_X1_X1_X1_EQ);
    put_instruction(code + 16, INSTRUCTION_SUBS_X4_X4_1);
...
static const struct benchmark_workload workloads[] = {
    ...
    {
        .name = "MSUB 画像减法与标志保持热点环",
        .instructions_per_iteration = 5,
        .x1_increment_per_iteration = 1,
        .iteration_register = 4,
        .initial_x6 = STORE_VALUE,
        .initial_x10 = UINT64_C(0xaaaaaaaacc996633),
        .expected_x4 = 0,
        .expected_x6 = 0,
        .fast_per_iteration = 5,
        .fallback_per_iteration = 0,
        .program_instruction_count = 6,
        .write_program = write_msub_program,
    },
    {
        .name = "LOAD SIMD pair 真实画像结果依赖热点环",
        .instructions_per_iteration = 6,
        .x1_increment_per_iteration = 1,
        .iteration_register = 4,
        .expected_x4 = 0,
...
static void reset_benchmark_data(
    ...
    store_little_endian(data + 0x350, 8, PAIR_FIRST_VALUE);
    store_little_endian(data + 0x358, 8, PAIR_SECOND_VALUE);
    store_little_endian(data + 0x360, 8, STORE_VALUE);
    store_little_endian(data + 0x368, 8,
            UINT64_C(0x1020304050607080));
}
