...
static qword_t reference_rbit(qword_t value, byte_t width) {
    assert(width == 32 || width == 64);
    value &= width == 32 ? UINT32_MAX : UINT64_MAX;
    qword_t result = 0;
    for (unsigned bit = 0; bit < (unsigned) width; bit++) {
        result = (result << 1U) | (value & UINT64_C(1));
        value >>= 1U;
    }
    return result;
}
...
static void assert_fast_rbit(dword_t instruction,
        struct cpu_state initial, byte_t rd, qword_t expected_value) {
    struct cpu_state expected = initial;
    expected.pc = CODE_PAGE + 4;
    expected.cycle++;
...
static void test_fast_rbit_differential(void) {
    unsigned case_count = 0;
    for (unsigned width_index = 0; width_index < 2; width_index++) {
        bool is_64 = width_index != 0;
        byte_t width = is_64 ? 64 : 32;
        for (unsigned bit = 0; bit < (unsigned) width; bit++) {
            qword_t source = UINT64_C(1) << bit;
            if (!is_64)
            ...
            aarch64_set_exclusive(&initial, DATA_PAGE + 0x40,
                    8, false, UINT64_C(0x1122), 0,
                    NULL, 3, 5, 7);
            assert_fast_rbit(encode_data_processing_1source(
                    is_64, 0, 4, 6), initial, 6,
                    reference_rbit(source, width));
            case_count++;
        }
    }
    assert(case_count == 96);
    ...
    assert(encode_data_processing_1source(
            false, 0, 2, 2) == INSTRUCTION_RBIT_W2_W2);
    assert(encode_data_processing_1source(
            false, 0, 3, 0) == INSTRUCTION_RBIT_W0_W3);
    ...
    initial.x[6] = UINT64_MAX;
    assert_fast_rbit(encode_data_processing_1source(
            true, 0, 4, 6), initial, 6, 0);
    case_count++;

    init_differential_cpu(&initial);
    initial.x[2] = UINT64_C(0xa5a5a5a5ffffffff);
    assert_fast_rbit(INSTRUCTION_RBIT_W2_W2,
            initial, 2, UINT32_MAX);
    case_count++;
    ...
    initial.x[6] = 0;
    assert_fast_rbit(encode_data_processing_1source(
            true, 0, 4, 6), initial, 6, UINT64_MAX);
    case_count++;

    init_differential_cpu(&initial);
    initial.x[0] = UINT64_MAX;
    initial.x[3] = UINT64_C(0xdeadbeef01234567);
    assert_fast_rbit(INSTRUCTION_RBIT_W0_W3,
            initial, 0, UINT32_C(0xe6a2c480));
    case_count++;

    init_differential_cpu(&initial);
    initial.x[7] = UINT64_C(0x0123456789abcdef);
    assert_fast_rbit(encode_data_processing_1source(
            true, 0, 7, 7), initial, 7,
...
static void test_fast_dispatch_structure(void) {
    ...
    _Static_assert(array_size(hot_instructions) + 2 == 66,
            "threaded 热点结构门必须覆盖全部 66 个 fast opcode");
    ...
    struct aarch64_decoded rbit_decoded;
    assert(aarch64_decode(INSTRUCTION_RBIT_W2_W2, &rbit_decoded));
    assert(rbit_decoded.opcode == AARCH64_OP_RBIT);
    assert(!seen_opcodes[rbit_decoded.opcode]);
    seen_opcodes[rbit_decoded.opcode] = true;
    for (unsigned index = 0; index < array_size(hot_instructions); index++)
        write_instruction(&fixture.tlb, CODE_PAGE + index * 4,
    ...
    write_instruction(&fixture.tlb, fallback_pc, INSTRUCTION_RBIT_W2_W2);
    qword_t rbit_sp = cpu.sp;
    dword_t rbit_nzcv = cpu.nzcv;
    cpu.x[2] = UINT64_C(0xa5a5a5a501234567);
    assert(run_at(&runner, &cpu, fallback_pc).stop ==
            AARCH64_STEP_RETIRED);
    assert(cpu.x[2] == UINT32_C(0xe6a2c480));
    assert(cpu.sp == rbit_sp);
    assert(cpu.nzcv == rbit_nzcv);
    assert(cpu.pc == fallback_pc + 4);
    assert_stats(&runner, 47, array_size(hot_instructions) + 3,
            array_size(hot_instructions) + 48, 2);

    cpu.x[2] = UINT64_C(0xa5a5a5a589abd000);
    assert(run_at(&runner, &cpu, fallback_pc).stop ==
            AARCH64_STEP_RETIRED);
    assert(cpu.x[2] == UINT32_C(0x000bd591));
    assert(cpu.sp == rbit_sp);
    assert(cpu.nzcv == rbit_nzcv);
    assert(cpu.pc == fallback_pc + 4);
    assert_stats(&runner, 48, array_size(hot_instructions) + 3,
            array_size(hot_instructions) + 49, 2);
    const struct aarch64_threaded_stats *stats =
            aarch64_runner_threaded_stats(&runner);
...
static void test_profile_aggregation(void) {
    ...
    assert(snapshot.fallback_by_opcode[AARCH64_OP_RBIT] == 0);
