...
static void test_fast_dispatch_structure(void) {
    ...
    _Static_assert(array_size(hot_instructions) == 60,
            "threaded 热点结构门必须覆盖全部 60 个 opcode");
    ...
    assert(cpu.x[0] == DATA_PAGE + 0x788);
    ...
    assert(cpu.v[31].d[1] == 0);
    assert(cpu.nzcv == UINT32_C(0x50000000));
    assert_stats(&runner, 40, array_size(hot_instructions) + 1,
    ...
    assert(cpu.nzcv == UINT32_C(0x50000000));
    assert_stats(&runner, 41, array_size(hot_instructions) + 1,
    ...
    assert(cpu.nzcv == UINT32_C(0x50000000));
    assert_stats(&runner, 42, array_size(hot_instructions) + 1,
...
static void test_profile_aggregation(void) {
    ...
    first_cpu.x[0] = DATA_PAGE + 0x100 - 56;
    first_cpu.v[31].d[0] = UINT64_C(0x0123456789abcdef);
    first_cpu.v[31].d[1] = UINT64_C(0xfedcba9876543210);
    ...
    assert(memcmp(first_fixture.memory.data + 0x100,
            &first_cpu.v[31], sizeof(first_cpu.v[31])) == 0);
    assert(first_cpu.x[0] == DATA_PAGE + 0x100 - 56);
    assert(first_cpu.sp == UINT64_C(0x1000));
    assert(first_cpu.nzcv == UINT32_C(0x30000000));

    first_cpu.sp = UINT64_C(0x2000);
    first_cpu.x[0] = DATA_PAGE + 0x140 - 56;
    first_cpu.v[31].d[0] = UINT64_C(0x8877665544332211);
    first_cpu.v[31].d[1] = UINT64_C(0x1020304050607080);
    first_cpu.nzcv = UINT32_C(0xa0000000);
    assert(run_at(&first_runner, &first_cpu, CODE_PAGE).stop ==
            AARCH64_STEP_RETIRED);
    assert(memcmp(first_fixture.memory.data + 0x140,
            &first_cpu.v[31], sizeof(first_cpu.v[31])) == 0);
    assert(first_cpu.x[0] == DATA_PAGE + 0x140 - 56);
    ...
    second_cpu.x[4] = DATA_PAGE + 0x210;
    second_cpu.v[0].d[0] = UINT64_C(0x1122334455667788);
    second_cpu.v[0].d[1] = UINT64_C(0x99aabbccddeeff00);
    ...
    assert(memcmp(second_fixture.memory.data + 0x200,
            &second_cpu.v[0], sizeof(second_cpu.v[0])) == 0);
    assert(second_cpu.x[4] == DATA_PAGE + 0x210);
