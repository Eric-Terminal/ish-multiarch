...
static void execute_advsimd_compare_zero_scalar(struct cpu_state *cpu,
        const struct aarch64_decoded *instruction) {
    byte_t rd = instruction->operands.advsimd_unary.rd;
    byte_t rn = instruction->operands.advsimd_unary.rn;
    qword_t source = cpu->v[rn].d[0];
    union aarch64_vector_reg result = {0};
    if ((source & (UINT64_C(1) << 63)) == 0)
        result.d[0] = UINT64_MAX;
    // 延迟整体写回保护自别名，并清零标量目标的高 64 位。
    cpu->v[rd] = result;
    cpu->pc += 4;
}
...
static void execute_advsimd_reverse(struct cpu_state *cpu,
    ...
    for (byte_t block = 0; block < blocks; block++) {
        byte_t first = (byte_t) (block * elements_per_block);
        for (byte_t element = 0; element < elements_per_block; element++) {
            qword_t value = read_vector_element(&source, element_size,
                    (byte_t) (first + elements_per_block - 1 - element));
            write_vector_element(&result, element_size,
...
static bool execute_simd_single_lane(struct cpu_state *cpu,
        ...
        struct guest_memory_fault *fault) {
    assert(tlb != NULL);
    byte_t rt = instruction->operands.advsimd_single_lane.rt;
    byte_t rn = instruction->operands.advsimd_single_lane.rn;
    byte_t element_size =
            instruction->operands.advsimd_single_lane.element_size;
