...
static void test_growth_and_shrink(void) {
    ...
    aarch64_linux_mm_init(&memory, &table, BRK_BASE, BRK_LIMIT);
    assert(aarch64_linux_brk(&memory, 0) == BRK_BASE);
    assert(aarch64_linux_brk(&memory, BRK_BASE - 1) == BRK_BASE);

    qword_t generation = table.address_space.generation;
    assert(aarch64_linux_brk(&memory, BRK_LIMIT + 1) == BRK_BASE);
    ...
    assert(aarch64_linux_brk(&memory, BRK_BASE + 64) == BRK_BASE + 64);
    ...
    first_page[0] = 0x5a;
    generation = table.address_space.generation;
    assert(aarch64_linux_brk(&memory, BRK_BASE + 128) == BRK_BASE + 128);
    assert(table.address_space.generation == generation);
    assert(first_page[0] == 0x5a);

    assert(aarch64_linux_brk(&memory,
    ...
    assert(aarch64_linux_brk(&memory, BRK_BASE) == BRK_BASE);
    assert_not_mapped(&table, BRK_BASE);
...
static void test_address_limit(void) {
    ...
    assert(guest_page_table_init(&table, 48));
    struct aarch64_linux_mm memory;
    aarch64_linux_mm_init(&memory, &table, TOP_PAGE,
            (UINT64_C(1) << 48) - 1);
    assert(aarch64_linux_brk(&memory, TOP_PAGE + 1) == TOP_PAGE + 1);
    assert(aarch64_linux_brk(&memory, UINT64_C(1) << 48) == TOP_PAGE + 1);
    assert(aarch64_linux_brk(&memory, UINT64_MAX) == TOP_PAGE + 1);
    assert(aarch64_linux_brk(&memory, TOP_PAGE) == TOP_PAGE);
