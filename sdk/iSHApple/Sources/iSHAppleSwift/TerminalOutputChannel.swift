...
final class TerminalOutputChannel: @unchecked Sendable {
  private typealias Continuation =
    CheckedContinuation<TerminalOutputEvent?, any Error>

  private let condition = NSCondition()
  private let capacity: Int
  private var queue: [TerminalOutputEvent] = []
  ...
  func send(_ event: TerminalOutputEvent) {
  ...
  func next() async throws -> TerminalOutputEvent? {
    try await withCheckedThrowingContinuation { continuation in
      var immediate: Result<TerminalOutputEvent?, any Error>?
